...
struct Span
{
	PAGE_ID _pageId = 0;				//大页内存起始页号
	size_t _n = 0;						//页的数量

	Span* _next = nullptr;			//双向循环
	Span* _prev = nullptr;			//带头链表

	size_t _objSize = 0;			//切好的小对象的大小
	size_t _useCount = 0; 			//引用计数
	void* _freeList = nullptr;		//切好小块内存的自由链表
	bool _isUse = false;			//判断是否仍在使用
};
...
public:
	...
	std::mutex& Mtx() { return _mtx; }//返回桶锁
